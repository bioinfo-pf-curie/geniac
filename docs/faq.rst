...
The :ref:`run-profile-conda` relies on the ``environment.yml`` that is automatically generated by `geniac`. However, building a |conda| recipe can sometimes tricky as the order of the channels and the dependencies matters. `geniac` can not guess what is the appropriate order. Moreover, sometimes |conda| may want to solve conflicts between incompatible packages. Thus, you will have to correct the ``environment.yml`` file manually, add it the git repository (where is located the ``main.nf`` file) and install the pipeline with the following options:
