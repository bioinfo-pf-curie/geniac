...
::
   ...
   │   ├── helloWorld
   ├── nextflow.config
   ├── nf-modules                   # nextflow files for process and subworkflows
   │   └── local
   │       ├── process
   │       │   ├── alpine.nf
   │       │   ├── checkDesign.nf
...
How can I compare the containers form one version of the pipeline to another?
=============================================================================

Geniac computes a sha256sum signature for each tool which takes into account:

* the recipe (.def or .Dockerfile) generated by Geniac

* the conda recipe (.yml files) provided in the ``recipes/conda`` folder

* the content of the ``recipes/dependencies`` folder
