...
| The user password should be set in your environment using the comnmand `nextflow secrets set DOCKER_REGISTRY_PUSH_USER your_password`
| Default is empty string ''."
...
| BOOL
| Generate and install Singularity def files and images. The images are built using the containers avaibable on the registry defined by the variable ap_docker_push_registry. This requires that geniac has been previously run using both :ref:`install-ap_install_docker_images` and :ref:`install-ap_push_images` options.
| Default is OFF.
