...
| STRING
| Docker registry used to build the containers from recipes automatically generated by geniac (to bootstrap from ``centos:7`` and ``conda/miniconda3-centos7`` containers).
| Default is empty (which means that the official docker hub registry is used).
