...
### Geniac variables
GENIAC_GITHUB="https://github.com/bioinfo-pf-curie/geniac"
...
class GeniacBase(ABC, LogMixin):
    ...
    def init_working_src_folder(self, git_branch: str):
        ...
        # Else try to clone project.metadata.url into src folder
            ...
            ######################################################################
            geniac_tar_url = f"{GENIAC_GITHUB}/archive/refs/tags/version-{__version__}.tar.gz"
            geniac_tar_path = self.working_dirs["src"].as_posix() + "/geniac.tar.gz"
            request.urlretrieve(geniac_tar_url, geniac_tar_path)
            geniac_tar = tarfile.open(geniac_tar_path)
            geniac_tar.extractall(path=self.working_dirs["src"].as_posix())
            geniac_tar.close()

            if os.path.isdir(self.working_dirs["src"].as_posix()+'/geniac'):
                os.remove(geniac_tar_path)
                shutil.rmtree(self.working_dirs["src"].as_posix()+'/geniac')
                shutil.move(self.working_dirs["src"].as_posix() + f"/geniac-version-{__version__}", self.working_dirs["src"].as_posix() + "/geniac")
                self.info(f"The geniac folder bas been replaced using geniac version-{__version__} from {geniac_tar_url}.")
            else:
                shutil.move(self.working_dirs["src"].as_posix() + f"/geniac-version-{__version__}", self.working_dirs["src"].as_posix() + "/geniac")
                os.remove(geniac_tar_path)
                self.info(f"The geniac directory does not exist. It has been created using geniac version-{__version__} from {geniac_tar_url}.")
        else:
